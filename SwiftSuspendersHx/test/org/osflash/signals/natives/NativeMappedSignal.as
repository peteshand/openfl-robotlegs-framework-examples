package org.osflash.signals.natives
{
	 ...
	 */
	class NativeMappedSignal extends NativeRelaySignal
	{
		/**
		...
		private function get mappingFunction ():Function 
		{
		 ...
		 * 
		 * <listing version="3.0">
		 *  signal = new NativeMappedSignal(button, MouseEvent.CLICK, MouseEvent, String, Int, Float).mapTo("ping", 3, 3.1415)
		 *  signal.add(function(arg1:String, arg2:Int, arg3:Float):Void { trace(arg1, arg2, arg3) }) // prints "ping", 3, 3.1415
		 * </listing>
		 * 
		 * If the argument is a function then it is called when the event this NativeMappedSignal is listening for is dispatched.
		 * The function should return an Array<Dynamic> or a single object. The data returned from the function is passed along as arguments in the Signal dispatch.
		 ...
		 * 
		 * <listing version="3.0">
		 *  signal = new NativeMappedSignal(button, MouseEvent.CLICK, MouseEvent, String, Int, Float).mapTo(function():Void { 
		 ...
		 * @return An object or Array<Dynamic> of objects mapped from an Event. The mapping of Event to data will be performed by the mapping function
		 * if it is set. A list of object literals can also be supplied in place of the mapping function.
		 * If no mapping function or object literals are supplied then an empty Array<Dynamic> is returned or
		 * if <code>valueClasses.length > 0</code> an ArgumentError is thrown.
		 *
		 * @see #mapTo()
		 */
		private function mapEvent(eventFromTarget:Event):Object
		{
		...
		override public function dispatchEvent(event:Event):Bool
		{
			...
			if (mappedData is Array<Dynamic>)
			{
				const valueObjects:Array<Dynamic> = mappedData as Array<Dynamic>;
		...
		override private function onNativeEvent(event:Event):Void
		{
			const mappedData:Object = mapEvent(event);
			...
			if (mappedData is Array<Dynamic>)
			{
				if (valueClasses.length == 1 && valueClasses[0] == Array<Dynamic>)//TODO invariant
				{
				...
				{
					const mappedDataArray:Array<Dynamic> = mappedData as Array<Dynamic>;
