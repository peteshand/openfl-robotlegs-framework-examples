package com.imag.core.utils.colour 
{
	public class ColourUtil
	{
		...
		public static function combineColours(colours:Vector.<uint>, ratios:Vector.<Number>):uint 
		{
			var alphas:Vector.<uint> = new Vector.<uint>();
			var reds:Vector.<uint> = new Vector.<uint>();
			var blues:Vector.<uint> = new Vector.<uint>();
			var greens:Vector.<uint> = new Vector.<uint>();
			
			for (var i:int = 0; i < colours.length; i++) 
			{
				alphas.push((colours[i] >> 24) & 0xFF);
				reds.push((colours[i] >> 16) & 0xFF);
				blues.push((colours[i] >> 8) & 0xFF);
				greens.push(colours[i] & 0xFF);
			}
			
			var combinedAlpha:uint = combineChannel(alphas, ratios);
			var combinedRed:uint = combineChannel(reds, ratios);
			var combinedGreen:uint = combineChannel(blues, ratios);
			var combinedBlue:uint = combineChannel(greens, ratios);
			
			return (combinedAlpha << 24) + (combinedRed << 16) + (combinedGreen << 8) + combinedBlue;
		}
		
		public static function combineChannel(colours:Vector.<uint>, ratios:Vector.<Number>):uint 
		{
			var returnVal:uint = 0x0;
			for (var i:int = 0; i < colours.length; i++) 
			{
				returnVal += colours[i] * ratios[i];
			}
			return returnVal;
		}
	}
